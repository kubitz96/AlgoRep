...
int load_matches_from_file(char filename[], struct Configurazione **start)
{
	...
	{	
		...
		tmp.vincitore = the_winner_is(tmp.configurazione);
		tmp.punteggio = match_score(tmp.configurazione);
...
void print_list(struct Configurazione *start)
{
	if(start != NULL)
	{
		for (int i = 0; i < 3; ++i)
		{
			for (int j = 0; j < 3; ++j)
			{
			printf("%c", start->Deposito->configurazione[i][j]);
			}
		}
	
		printf(" %c %d \n", start->Deposito->vincitore, start->Deposito->punteggio);
...
char the_winner_is(char Conf[3][3])
{
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		if(checkriga == 1) return Conf[i][0];
...
int write_winners_on_file(char filename[], char filename2[], struct Configurazione *start) //non passo per riferimento poiche non devo modifica la lista ma solo copiarla
{
	...
	if(fp2 == NULL) errorManager(ERR_W);
	
		struct Configurazione* cursore = start;
		
		while(cursore != NULL)
		{
			if(cursore->Deposito->vincitore == 'X')
			{
				for (int i = 0; i < 3; ++i)
				{
					for (int j = 0; j < 3; ++j)
					{
						fprintf(fp,"%c",cursore->Deposito->configurazione[i][j]);
					}
				}
				fprintf(fp," %c %d \n", cursore->Deposito->vincitore, cursore->Deposito->punteggio);
			}

			else if(cursore->Deposito->vincitore == 'O')
			{
				for (int i = 0; i < 3; ++i)
				{
					for (int j = 0; j < 3; ++j)
					{
						fprintf(fp2,"%c",cursore->Deposito->configurazione[i][j]);
					}
				}
				fprintf(fp2," %c %d \n", cursore->Deposito->vincitore, cursore->Deposito->punteggio);
