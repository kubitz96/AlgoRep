...
void allocAgenda(int Neventi, int ***eventi, char ***impegni)
{
	...
	ev = malloc(Neventi*sizeof(int *));
	if (ev == NULL) {
		gestisciErrore(MEMORY_ALLOC_ERROR);
	}
	
	ev[0] = malloc(Neventi*NCAMPI*sizeof(int));
	...
	imp = malloc(Neventi*sizeof(char *));
	
	for (i=0; i<Neventi; i++) {
		imp[i] = malloc(LUNGHEZZASTR*sizeof(char));
	}
...
int *findLongest(int **eventi, int Neventi)
{
	...
	indiciEventi = calloc(Neventi, sizeof(int));
